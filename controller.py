...
def event_manage(event, field, pressed_mouse, interface, speed, settings):
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        ...
        if event.key == pygame.K_BACKSPACE and interface.name_of_file != '':   # Backspace
...
def menu_event_manage(event, main_menu, pressed_mouse, Main_menu, Game, field_size):
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        ...
        if mouse_pos_check(array(pygame.mouse.get_pos()), main_menu.large_field.bg_rect):
            ...
            main_menu.small_field.pressed = 0
            main_menu.middle_field.pressed = 0
        if mouse_pos_check(pygame.mouse.get_pos(), main_menu.upload.bg_rect):
            if main_menu.upload.pressed == 0:
    ...
    return Game, Main_menu, pressed_mouse, field_size, main_menu.name_of_file
